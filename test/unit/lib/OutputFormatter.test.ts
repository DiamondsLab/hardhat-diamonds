...
describe("OutputFormatter", () => {
  ...
  describe("generateSummaryHeader", () => {
    ...
    it("should include warning section", () => {
      ...
      expect(result).to.include("AUTO-GENERATED FILE - DO NOT EDIT MANUALLY");
      expect(result).to.include("This file was automatically generated by hardhat-diamonds.");
      expect(result).to.include("Any manual changes will be lost on the next regeneration.");
    });
    ...
    it("should have aligned box-drawing borders", () => {
      ...
      const result = formatter.generateSummaryHeader(options);
      const lines = result.split("\n");
      const contentLines = lines.slice(1, -1).map(line => line.replace(/^ \* /, ""));
      ...
      // Verify all content lines are exactly 80 characters
      contentLines.forEach((line, idx) => {
        expect(line.length, `Line ${idx} should be 80 chars: "${line}"`).to.equal(80);
      });
    });
