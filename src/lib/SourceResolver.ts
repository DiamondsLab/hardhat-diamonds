...
export class SourceResolver {
   ...
   * @param sourceFilePath - Path of the source file (for error messages)
   * @returns Array of parsed import information
   * @private
   */
  private parseImports(content: string, sourceFilePath: string): ImportInfo[] {
    ...
    while ((match = importRegex.exec(content)) !== null) {
      ...
      const isNodeModule = importPath.startsWith("@") || 
  ...
  private async resolveImportPath(
    ...
    try {
      ...
      if (this.verbose) {
        console.log(`[SourceResolver] Resolved ${importPath} -> ${resolvedPath}`);
      }
