	...
	if (ethers) {
		...
		try {
			...
			try {
				...
				return createDiamondContract<T>(diamond, contractAddress, ethers.provider);
			}
		}
	}
...
export async function createDiamondContract<T>(
	...
	if (!signerOrProvider) {
		throw new Error('Either ethers or signerOrProvider must be provided to createDiamondContract');
	}

	return new Contract(contractAddress, diamondAbiArtifact.abi, signerOrProvider) as T;
}
