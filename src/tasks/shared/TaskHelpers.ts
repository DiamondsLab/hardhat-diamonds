...
export class ProgressIndicator {
  ...
  start(): void {
    ...
    this.interval = setInterval(() => {
      process.stdout.write(`\r${chalk.cyan(this.steps[this.currentStep])} ${this.message}`);
...
export class TaskHelpers {
  ...
  convertToGenerationOptions(args: DiamondAbiTaskArgs): DiamondAbiGenerationOptions {
    return {
      ...
      outputDir: args.outputDir || join(this.hre.config.paths.root, 'diamond-abi'),
  ...
  logTaskCompletion(taskName: string, duration: number, success: boolean): void {
    ...
    if (success) {
      console.log(chalk.green(`✅ ${taskName} completed successfully in ${durationStr}`));
    } else {
  ...
  normalizeTaskArgs<T extends DiamondAbiTaskArgs>(args: T): T {
    ...
    // Apply defaults
    normalized.outputDir = normalized.outputDir || join(this.hre.config.paths.root, 'diamond-abi');
    (normalized as any).network = (normalized as any).targetNetwork || this.hre.network.name;
    ...
    if ('typechainTarget' in normalized) {
      ...
      typechainArgs.typechainOutDir = typechainArgs.typechainOutDir || join(this.hre.config.paths.root, 'diamond-typechain-types');
    }
