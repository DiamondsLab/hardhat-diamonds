...
task("diamond:generate-abi", "Generate Diamond ABI from configuration or deployment data")
  ...
  .setAction(async (args: DiamondAbiTaskArgs, hre: HardhatRuntimeEnvironment) => {
    ...
    try {
      ...
      if (!normalizedArgs.verbose) {
        progressIndicator = new ProgressIndicator(`Generating Diamond ABI for ${normalizedArgs.diamondName}...`);
      ...
      if (result.outputPath && existsSync(result.outputPath)) {
        try {
          ...
          console.log(chalk.blue(`   File size: ${TaskHelpers.formatFileSize(stats.size)}`));
        } catch {
      ...
      if (normalizedArgs.verbose) {
        ...
        console.log(chalk.cyan(`   Average time per facet: ${TaskHelpers.formatDuration(Math.round(duration / Math.max(result.stats.facetCount, 1)))}`));
      }

      // Success message with next steps
      console.log(chalk.green('\n🎉 Diamond ABI generation complete!'));
      console.log(chalk.cyan('💡 Next steps:'));
      console.log(chalk.cyan('   - Use the generated ABI in your dApp frontend'));
      console.log(chalk.cyan('   - Generate TypeScript types with: npx hardhat diamond:generate-abi-typechain'));
      console.log(chalk.cyan('   - Import the ABI artifact in your tests and scripts'));
