...
task("diamond:generate-abi-typechain", "Generate Diamond ABI and TypeScript types using TypeChain")
  ...
  .setAction(async (args: DiamondAbiTypechainTaskArgs, hre: HardhatRuntimeEnvironment) => {
    ...
    try {
      ...
      if (!argsValidation.isValid) {
        TaskValidation.formatValidationResult(argsValidation, normalizedArgs.verbose);
      ...
      if (!configValidation.isValid) {
        TaskValidation.formatValidationResult(configValidation, normalizedArgs.verbose);
      ...
      if (!systemValidation.isValid) {
        TaskValidation.formatValidationResult(systemValidation, normalizedArgs.verbose);
      ...
      if (!normalizedArgs.verbose) {
        progressIndicator = new ProgressIndicator(`Generating Diamond ABI and TypeScript types for ${normalizedArgs.diamondName}...`);
      ...
      if (normalizedArgs.verbose) {
        ...
        console.log(chalk.cyan(`   Chain ID: ${networkInfo.chainId || 'unknown'}`));
        console.log(chalk.cyan(`   Is Local: ${networkInfo.isLocal ? 'yes' : 'no'}`));
        if (networkInfo.url) {
      ...
      if (normalizedArgs.verbose) {
        ...
        console.log(chalk.cyan(`   Functions: ${abiResult.stats.totalFunctions}`));
      ...
      if (abiResult.stats.duplicateSelectorsSkipped > 0) {
        console.log(chalk.yellow(`   Duplicates skipped: ${abiResult.stats.duplicateSelectorsSkipped}`));
      }
      ...
      if (existsSync(abiResult.outputPath)) {
        try {
          ...
          console.log(chalk.cyan(`   ABI file size: ${TaskHelpers.formatFileSize(abiStats.size)}`));
        } catch {
      ...
      console.log(chalk.cyan(`   Output directory: ${typechainResult.outputDir}`));
      console.log(chalk.cyan(`   Generated files: ${typechainResult.generatedFiles.length}`));
      ...
      if (existsSync(typechainResult.outputDir)) {
        try {
          const files = readdirSync(typechainResult.outputDir, { recursive: true });
      ...
      if (normalizedArgs.verbose && typechainResult.generatedFiles.length > 0) {
        ...
        if (typechainResult.generatedFiles.length > 10) {
          console.log(chalk.cyan(`   ... and ${typechainResult.generatedFiles.length - 10} more files`));
        }
      }
      ...
      if (normalizedArgs.verbose) {
        ...
        console.log(chalk.cyan(`   Total processing time: ${TaskHelpers.formatDuration(duration)}`));
        console.log(chalk.cyan(`   Average time per facet: ${TaskHelpers.formatDuration(Math.round(duration / Math.max(abiResult.stats.facetCount, 1)))}`));
        console.log(chalk.cyan(`   TypeChain files per second: ${Math.round(typechainResult.generatedFiles.length / (duration / 1000))}`));
      }

      // Success message with next steps
      console.log(chalk.green('\n🎉 Diamond ABI and TypeScript generation complete!'));
      console.log(chalk.cyan('\n💡 Next steps:'));
      console.log(chalk.cyan('   - Import the generated TypeScript types in your frontend code'));
      console.log(chalk.cyan('   - Use the types for type-safe contract interactions'));
      console.log(chalk.cyan('   - Check the generated factory classes for contract deployment'));
      console.log(chalk.cyan(`   - Example: import { ${normalizedArgs.diamondName}__factory } from "${typechainResult.outputDir}"`));
      ...
      console.log(chalk.cyan('   - Verify diamond configuration exists in hardhat.config.ts'));
      console.log(chalk.cyan('   - Check that diamond name is spelled correctly'));
      console.log(chalk.cyan('   - Ensure contract artifacts are compiled (run: npx hardhat compile)'));
      console.log(chalk.cyan('   - Verify TypeChain dependencies are installed'));
      console.log(chalk.cyan('   - Run with --verbose flag for detailed error information'));

      // Check for common issues
      const availableDiamonds = helpers.getAvailableDiamonds();
      if (availableDiamonds.length > 0) {
        console.log(chalk.cyan(`   - Available diamonds: ${availableDiamonds.join(', ')}`));
      } else {
        console.log(chalk.cyan('   - No diamond configurations found, add them to hardhat.config.ts'));
      }

      // TypeChain-specific suggestions
      console.log(chalk.cyan('\n🔧 TypeChain-specific tips:'));
      console.log(chalk.cyan('   - Install TypeChain: npm install --save-dev typechain @typechain/ethers-v6'));
      console.log(chalk.cyan('   - Check TypeChain target compatibility with your project'));
      console.log(chalk.cyan('   - Ensure ethers.js is installed for ethers-v6 target'));
